...
#			  : When this program is larger than 2377, it will be the single lagrest#
#					 program.                                                       #
#                                                                                   #
# Get more Tcl/Tk scripts from http://www.geocities.com/binnyva/code/tcl            #
#####################################################################################
...
set themes_folder [file join $home_folder "Themes"]
set user_folder   [file join $home_folder "User Files"]
...
proc loadThemes { page } {
	...
	while { $flag < $total_folders } {
		...
		catch { cd [file join $themes_folder "$folder"] }
		#Get the list of themes. They have the extension '.thm'
		set files [lsort [glob -nocomplain "*.thm"]]
		foreach i $files {
			if { $i != "[file join $themes_folder $folder .]" && $i != "[file join $themes_folder $folder ..]" } {
				if {![file isdirectory $i]} {
...
##############################################
					...
					lappend this_theme "[file join $themes_folder $folder $i]"
	...
	switch $page_no {
		...
		#Files
			...
			if { $seven < 1 } {
				...
				$tm.slb clear
				foreach part $parts {
					set file_name [file join $themes_folder "$part"]
...
proc saveTheme { page } {
	...
	set theme_file [file join $themes_folder "User" "$theme_name.thm"]
	set out [open "$theme_file" w]
	puts $out "$data"
	close $out
	
	tk_messageBox -message "Theme '$theme_name' saved to '$theme_file'"
}
...
proc addDir {page} {
	...
	if { $folder != "" } {
		...
		while { ($flag < $total_folders) && ($list_subfolders) } {
			...
			set files [lsort [glob -nocomplain [file join folder .*] [file join $folder *]]]
			foreach i $files {
				if { $i != "[file join $folder .]" && $i != "[file join $folder ..]" } {
					if {[file isdirectory $i]} {
						#If the specifed file is a directory, add it to the list of
						#	directories of which's file listing we should get.
						lappend dirs $i
...
#PORTABILITY PROBLEM : What if another platform has to use '\' instead of '/' - use \[file join $folder Whatever\]
#SOLUTION : Use \[file seperator\] command
	...
	foreach part $files {
		...
		if {[file exists $file_name]} {
			if {[file isdirectory  "$folder"]} {
				...
				if { [file exists "[file join $folder $name]"] } {
					#Ask user if he want's to overwrite. Not very nessary. So I comment it out.
...
# ----------------------------------------------------------------------
set page [.tnb add -label "File Selection"]
set page1 $page

label $page.lbl_main -text {File Selection} -font $heading
#List Box
frame $page.frm_slb
...
# Next Version
# ------------
#1. A proper sorting algoritham. - Use fileutil::find
#2. Writing the result to multiple files
#3. Clear up the code.
...
#7. Replace the RegSubs in processing to split - will save a lot of time.
#8. 'Invalid' causes bugs - Run aphabetic theme as an example.
#9. Solve PORTABILITY PROBLEM - Seach for that text
#10.Show error if user don't have Iwidgets
#11.Support for other browsers
#12.%File_Number% option starts at 0. We need this AND an otpion that starts numbers at 1